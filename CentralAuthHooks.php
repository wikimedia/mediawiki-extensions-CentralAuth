...
class CentralAuthHooks {
	...
	static function attemptAddUser( $user ) {
		...
		wfDebugLog( 'Bug39996', __METHOD__ . ": creating new user ($userName)\n" );
		try {
			$status = $user->addToDatabase();
		} catch ( MWException $e ) {
			wfDebugLog( 'Bug39996', __METHOD__ . "User::addToDatabase for \"$userName\" threw an exception:"
				. " {$e->getMessage()}" );
			throw $e;
		}
		...
		if ( $status === null ) {
			...
			wfDebugLog( 'Bug39996', __METHOD__.": failed with message " . $status->getWikiText() . "\n" );
			return false;
		}
