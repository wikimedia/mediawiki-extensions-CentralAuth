...
class CentralAuthHooks {
	...
	static function onAbortNewAccount( $user, &$abortError ) {
		...
		if (
			...
			$renameCache->get( CentralAuthUser::memcKey( 'globalrename', sha1( $user->getName() ) ) )
		) {
	...
	static function attemptAddUser( $user ) {
		...
		if (
			...
			$renameCache->get( CentralAuthUser::memcKey( 'globalrename', sha1( $userName ) ) )
		) {
	...
	static function onAbortLogin( $user, $password, &$retval ) {
		global $wgCentralAuthRenameObjectStore;
		$renameCache = ObjectCache::newFromParams( $wgCentralAuthRenameObjectStore );
		$wikis = $renameCache->get( CentralAuthUser::memcKey( 'globalrename', sha1( $user->getName() ) ) );
