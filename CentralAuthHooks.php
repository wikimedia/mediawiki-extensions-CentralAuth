...
class CentralAuthHooks {
	...
	/**
	 * Get the username from CentralAuth for a list of CentralAuth user id's. This hook must
	 ...
	 */
	public static function onOAuthGetUserNamesFromCentralIds( $wgMWOAuthCentralWiki, &$namesById, $wgMWOAuthSharedUserSource ) {
		...
		foreach ( $namesById as $userid => $name ) {
			...
			if ( $centralUser->getId() == 0
				...
				wfDebugLog( 'CentralAuth', __METHOD__ . ": user '{$centralname}' cannot use OAuth on " . wfWikiID() );
				$name = false;
			} else {
