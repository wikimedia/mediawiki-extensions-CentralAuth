...
class CentralAuthIpReputationPreAuthenticationProvider extends AbstractPreAuthenticationProvider {
	...
	public function testForAccountCreation( $user, $creator, array $reqs ) {
		...
		if ( !isset( $data['risks'] ) || !isset( $data['tunnels'] ) ) {
			// 'risks' and 'tunnels' should always be set, but just in case, check and deny
			// without considering the risk/tunnel type config later on.
			// IP exists in ipoid.
			$this->logger->warning(
				'Got iPoid data for {ip} without "risks" or "tunnels", preventing user {user} from creating an account',
				[
					'user' => $user->getName(),
					'ip' => $ip,
			...
			$this->statsDataFactory->increment( "CentralAuth.IpReputation.$statsdKey.UNKNOWN" );
			if ( $shouldLogOnly ) {
				return StatusValue::newGood();
			}
			return StatusValue::newFatal( 'apierror-autoblocked' );
		}
		...
		if ( $blockedRisks ) {
			...
			// Not a completely accurate message, but it is close enough, and
			// also exists in translation on all wikis.
