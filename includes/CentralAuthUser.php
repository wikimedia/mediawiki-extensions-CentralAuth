...
class CentralAuthUser extends AuthPluginUser implements IDBAccessObject {
	 ...
	 * @param $method String
	 * @param $sendToRC bool
	 * @param string|bool $ts MediaWiki timestamp or false for current time
	 *
	 * Prerequisites:
	 * - completed migration state
	 */
	public function attach( $wikiID, $method = 'new', $sendToRC = true, $ts = false ) {
		$this->checkWriteMode();
		$dbw = CentralAuthUtils::getCentralDB();
		...
		$dbw->insert( 'localuser',
			array(
				'lu_wiki'               => $wikiID,
				'lu_name'               => $this->mName,
				'lu_attached_timestamp' => $ts ?: $dbw->timestamp(),
	...
	protected function localUserData( $wikiID ) {
		...
		if ( !$row ) {
			$lb->reuseConnection( $db );
			throw new LocalUserNotFoundException( "Could not find local user data for {$this->mName}@{$wikiID}" );
		}
