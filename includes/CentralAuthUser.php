...
class CentralAuthUser extends AuthPluginUser implements IDBAccessObject {
	...
	private $mBeingRenamed, $mBeingRenamedArray, $mGroupsExp;
	...
	protected function loadGroups() {
		...
		$res = $db->select(
			...
			[ 'ggp_group=gug_group', 'gug_user' => $this->getId(), 'gug_expiry IS NULL OR gug_expiry >= '
		...
		foreach ( $res as $row ) {
			...
			$this->mGroupsExp[$row->ggp_group] = $row->gug_expiry;
		}
	 ...
	 * @return array
	 */
	public function getGlobalexpGroups() {
		$this->loadGroups();

		return $this->mGroupsExp;
	}
	 ...
	 *
	 * @throws MWException
	 * @param string $group Valid GlobalGroup to add
	...
	public function addToGlobalGroup( $group, $expiry = null ) {
		...
		// Check that $group is a valid GlobalGroup
		if ( !in_array( $group, $allGroups ) ) {
			throw new UnexpectedValueException(
				'Don\'t try inserting a non valid global group' );
		}
		...
		if ( is_null( $expiry ) || ( $expiry && !$ugm->isExpired() ) ) {
			...
			throw new UnexpectedValueException( 'Any valid expiry provided' );
		}
