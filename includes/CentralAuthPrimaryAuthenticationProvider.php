	...
	extends AbstractPasswordPrimaryAuthenticationProvider
{
	...
	/** @var bool Whether to prevent new unattached accounts from being created */
	...
	public function beginPrimaryAuthentication( array $reqs ) {
		...
		if ( !$pass && $this->checkSULMigration ) {
			...
			if ( $renamedUsername !== false ) {
				...
				// Don't do any of the checks below if we checked for a renamed user
				return $centralUser->isAttached()
					? AuthenticationResponse::newFail( wfMessage( 'wrongpassword' ) )
	...
	public function testUserForCreation( $user, $autocreate ) {
		...
		if ( !$autocreate ) {
			...
			if ( class_exists( 'AntiSpoof' ) &&
				!CentralAuthAntiSpoofHooks::asAbortNewAccountHook( $user, $message )
			) {
				$status->fatal( 'rawmessage', $message );
			}
		}
