...
class GlobalUsersPager extends AlphabeticPager {
	...
	public function doBatchLookups() {
		...
		foreach ( $this->mResult as $row ) {
			...
			// no point in adding users that don't belong to any group
			if ( $row->gug_group ) {
				$groups[$row->gu_id] = array_combine(
					explode( '|', $row->gug_group ),
					explode( '|', $row->gug_expiry )
				);
	...
	protected function getUserGroups( $id, $username ) {
		...
		foreach ( $this->groupsMemberships[$id] as $group => $ugm ) {
			...
			if ( $ugm->getExpiry() !== 'null' ) {
				$expiryMsg = $this->getContext()
					->msg( 'group-membership-link-with-expiry' )
					->params(
						$wikitextLink,
						$uiLanguage->userTimeAndDate( $ugm->getExpiry(), $uiUser )
					)
					->text();
