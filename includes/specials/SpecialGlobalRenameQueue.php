...
class SpecialGlobalRenameQueue extends SpecialPage {
	...
	protected function doResolveRequest( $approved, $data ) {
		...
		if ( $request->userIsGlobal() ) {
			...
			$notifyEmail = self::getRemoteUserMailAddress(
		...
		if ( $status->isGood() ) {
			...
			if ( $request->save() ) {
				...
				self::sendNotificationEmail( $notifyEmail, $subject, $body );
			} else {
	 ...
	 * @return MailAddress
	 */
	protected static function getRemoteUserMailAddress( $wiki, $username ) {
		$lb = wfGetLB( $wiki );
	 ...
	 * @return Status
	 */
	protected static function sendNotificationEmail( MailAddress $to, $subject, $body ) {
