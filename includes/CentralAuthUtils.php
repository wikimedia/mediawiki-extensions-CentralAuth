	...
	public static function purgeExpired() {
		...
		if ( $wgUseCAUserGroupExpiry ) {
			...
			do {
				...
				$res =
					$dbw->select( 'global_user_groups', [ 'gug_group', 'gug_user', 'gug_expiry' ],
						[ 'gug_expiry < ' . $dbw->addQuotes( $dbw->timestamp() ) ], __METHOD__,
				...
				if ( $res->numRows() > 0 ) {
					...
					$dbw->delete( 'global_user_groups',
						$dbw->makeList( $deleteCond, $dbw::LIST_OR ), __METHOD__ );
