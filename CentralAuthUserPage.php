...
class CentralAuthUserPage extends Article {
	...
	private function getForeignUrl() {
		if ( $this->mForeignUrl === false ) {
			...
			$prefix = MWNamespace::getCanonicalName( NS_USER );
			$this->mForeignUrl = WikiMap::getForeignURL( $wgCentralAuthUserPageWiki, $prefix . ":" . $this->getTitle()->getText() );
		}
	...
	public function showMissingArticle() {
		...
		$header = wfMessage( 'centralauth-userpage-header' )->params(
			...
			WikiMap::getForeignURL( $wgCentralAuthUserPageWiki, '' )
		)->parse();
