...
class CentralAuthLocalRenameUserJob extends Job {
	...
	public function run() {
		...
		if ( $rus->rename() ) {
			...
			$wikiListKey = CentralAuthUser::memcKey( 'globalrename', $from );
			$wikiList = $wgMemc->get( $wikiListKey );
			...
			if ( count( $wikiList ) == 0 ) {
				...
				$db = wfGetLB( $this->params['startedFrom'] )->getConnection( DB_MASTER, array(), $this->params['startedFrom'] );
