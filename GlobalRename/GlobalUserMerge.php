...
class GlobalUserMerge {
	...
	public function merge( $reason ) {
		...
		foreach ( $this->oldCAUsers as $oldCAUser ) {
			$this->databaseUpdates->remove( $oldCAUser->getName(), $this->newCAUser->getName() );
		}
	...
	private function setRenameStatuses( array $wikis ) {
		...
		$this->renameuserStatus->begin();
		if ( $this->renameuserStatus->setStatuses( $rows ) !== count( $rows ) ) {
			$this->renameuserStatus->rollback();
