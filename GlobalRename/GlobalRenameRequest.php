...
class GlobalRenameRequest {
	...
	public static function userHasPendingRequest( $username, $wiki ) {
		...
		return ( $res === false ) ? false : $res->rq_id;
	}
	...
	public static function isNameAvailable ( $name ) {
		...
		// New user creation checks against local wiki only using an API request,
		// but we need to check against te central user table instead
