...
class SpecialCentralLogin extends UnlistedSpecialPage {
	...
	protected function doLoginStart( $token ) {
		...
		// Start an unusable placeholder session stub and send a cookies
		...
		// The secret is that of the login attempt tracked in the initiating local session.
	...
	protected function doLoginComplete( $token ) {
		...
		$centralUser->setGlobalCookies( $_SESSION[$skey]['remember'], false, $info['sessionId'] );
