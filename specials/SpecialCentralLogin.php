...
class SpecialCentralLogin extends UnlistedSpecialPage {
	...
	protected function doLoginStart( $token ) {
		...
		if ( isset( $session['name'] ) ) { // fully initialized session
			if ( $session['name'] !== $centralUser->getName() ) {
				...
				$url = wfExpandUrl( // redirects should not be relative per RFC 2396
		...
		// If the user has a stub session, error out if the names do not match up
			...
			if ( $session['pending_name'] !== $centralUser->getName() ) {
				throw new MWException( "Token user does not match session's pending_name." );
				return;
			}
		}
		...
		// Note: the "remember me" token must be deal with later (security).
		...
		// Use WikiReference::getFullUrl(), returns a protocol-relative URL if needed
		$url = wfExpandUrl( // redirects should not be relative per RFC 2396
			$wiki->getFullUrl( 'Special:CentralLogin/complete' )
		);
	...
	protected function doLoginComplete( $token ) {
		...
		if ( WebRequest::detectProtocol() === 'https' && !$attempt['stickHTTPS'] ) {
			...
			$url = $this->getFullTitle()->getFullUrl( '', false, PROTO_HTTP );
			$this->getOutput()->redirect( $url );
		} else {
