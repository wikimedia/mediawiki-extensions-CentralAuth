...
class SpecialCentralLogin extends UnlistedSpecialPage {
	...
	protected function doLoginComplete( $token ) {
		...
		// This is the wrong way to do it, but LoginForm doesn't allow forcing the redirect
		// to be http, if wgSecureLogin isn't set. So we'll hack around it here. Also, this
		// runs the AutoLogin as http if needed, so we make an attempt to SUL over http.
		// TODO: Update LoginForm to allow forcing http, and figure out AutoLogins via http
		if ( WebRequest::detectProtocol() === 'https'
			&& ( ( $wgSecureLogin && !$attempt['stickHTTPS'] )
			|| $attempt['originalProto'] === 'http' )
		) {
			// The user wants an HTTP redirect link (as well as other links) and
			// this is on HTTPS, so retry the completion step on the correct proto
