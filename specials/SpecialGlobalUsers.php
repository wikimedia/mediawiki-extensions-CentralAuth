...
class GlobalUsersPager extends UsersPager {
	...
	function formatRow( $row ) {
		...
		if ( !$this->including && $row->gu_registration ) {
			$date_created = $lang->userDate( $row->gu_registration, $user_host );
			$time_created = $lang->userTime( $row->gu_registration, $user_host );
			$created = $this->msg( 'usercreated', $date_created, $time_created, $user )->escaped();
		...
		return Html::rawElement( 'li', array(), $this->msg( 'centralauth-listusers-item', $user, $info)->parse().$created);
	}
