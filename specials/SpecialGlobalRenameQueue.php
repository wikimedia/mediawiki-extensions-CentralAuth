	...
	protected function doResolveRequest( $approved, $data ) {
		...
		if ( $approved ) {
			if ( $request->userIsGlobal() ) {
				...
				$status = Status::newFatal( 'globalrenamequeue-wip',
					'Accept for local-only users'
				);
			}
		}
		...
		if ( $status->isGood() ) {
			...
			$request->setPerformer( $this->getUser()->getId() );
			$request->setComments( $data['comments'] );
			if ( $request->save() ) {
				// TODO: send email to the user about the change in status.
				// Can we just use User::sendMail() to do this? The user may
				// be a local-only user on another wiki.
			} else {
