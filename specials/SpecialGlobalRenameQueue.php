...
class SpecialGlobalRenameQueue extends SpecialPage {
	...
	protected function handleProcessRequest( array $pathArgs ) {
		...
		$req = GlobalRenameRequest::getRequest( $rqId );
		if ( $req === false ) {
		...
		if ( $req->status !== GlobalRenameRequest::PENDING ) {
			$action = 'status';
		}
	...
	protected function doShowRequestStatus( $req ) {
		$this->commonPreamble( 'globalrenamequeue-request-status-title',
			array( $req->name, $req->newname )
		);
		// TODO: implement display of request
		$this->getOutput()->addHtml(
			'<pre>' . htmlspecialchars( print_r( $req, true ) ) . '</pre>'
		);
	}
	 ...
	 *
	 * @param array $req Pending request
	 */
	protected function doShowProcessForm( $req ) {
		$this->commonPreamble(
			'globalrenamequeue-request-title', array( $req->name )
		);
		...
		// TODO: what wiki should we use as the home wiki for global request?
		// ... probably the CentralAuth user's home wiki I guess.
		$homeWiki = $req->wiki ?: wfWikiID();
		$headerMsg = $this->msg( 'globalrenamequeue-request-header',
			WikiMap::getForeignURL( $homeWiki, "User:{$req->name}" ),
			$req->name,
			$req->newname
		);
		$form->addHeaderText( $headerMsg->parseAsBlock() );

		$form->addHeaderText( $this->getUserInfoMessage(
			$homeWiki, $req->name, $req->wiki === null
		)->parseAsBlock() );

		if ( $req->reason ) {
			$reason = MessageCache::singleton()->parse(
				$req->reason, null, false
			)->getText();
		} else {
			$reason = $this->msg(
		...
		if ( $status instanceof Status && $status->isOk() ) {
			$this->getOutput()->redirect(
				$this->getPageTitle(
					self::PAGE_PROCESS_REQUEST . "/{$req->id}/{$status->value}"
				)->getFullURL()
			);
		}
	}
	...
	/**
	 * @param string $wiki Home wiki
	 * @param string $user Username
	 * @param bool $global Is user global?
	 * @return Message
	 */
	protected function getUserInfoMessage( $wiki, $user, $global ) {
		$userEnc = urlencode( $user );
		$talk = WikiMap::getForeignURL( $wiki, "User_talk:{$user}" );
		$contrib = WikiMap::getForeignURL(
			$wiki, "Special:Contributions/{$user}"
		);
		$logs = WikiMap::getForeignURL( $wiki, 'Special:Log') . "?page=User:{$userEnc}";
		$sulInfo = '//tools.wmflabs.org/quentinv57-tools/tools/sulinfo.php?username=' .
			$userEnc;
		$globalContrib = '//tools.wmflabs.org/guc/?blocks=true&user=' .
			$userEnc;
		if ( $global ) {
			$msg = $this->msg(
				'globalrenamequeue-request-userinfo-global',
				$talk, $contrib, $logs, $sulInfo, $globalContrib,
				WikiMap::getForeignURL( $wiki, "Special:CentralAuth/{$user}" )
			);
		} else {
			$msg = $this->msg(
				'globalrenamequeue-request-userinfo',
				$talk, $contrib, $logs, $sulInfo, $globalContrib
			);
		}
		return $msg;
	...
	protected function doResolveRequest( $approved, $data ) {
		if ( $approved ) {
			// TODO: what all needs to be done here?
			// If the user is local-only:
			// * rename the local user
			// * promote to a global user
			// If the user is a global user:
			// * trigger a global rename job
			$status = Status::newFatal( 'globalrenamequeue-wip', 'Accept' );
		} else {
			// TODO: mark request as denied --
			// rq_id => data['rid']
			// status => GlobalRenameRequest::REJECTED
			// completed => now
			// perfomer => $user
			// comments => data['comments']
			$status = Status::newFatal( 'globalrenamequeue-wip', 'Reject' );
		}
		// TODO: send email to the user about the change in status
		return $status;
	}
}
...
class RenameQueueTablePager extends TablePager {
	...
	public function formatValue( $name, $value ) {
		...
		switch ( $name ) {
			...
			case 'rq_completed_ts':
				...
				break;
			case 'rq_performer':
				// TODO: make a link to the user's page
				// value is CA user id, how do we make a CentralAuthUser from
				// that?
				break;
			case 'row_actions':
	...
	protected function formatActionValue( $row ) {
		return Html::element( 'a',
			array(
				'href' => $this->mOwner->getPageTitle(
					SpecialGlobalRenameQueue::PAGE_PROCESS_REQUEST . '/' . $row->rq_id
				)->getFullURL(),
				'class' => 'mw-ui-progressive',
			),
			$this->msg( 'globalrenamequeue-action-address' )->text()
		);
	}
	...
	public function getFieldNames() {
		if ( $this->mFieldNames === null ) {
			$this->mFieldNames = array(
				'rq_name' => $this->msg( 'globalrenamequeue-column-rq_name' )->text(),
				'rq_newname' => $this->msg( 'globalrenamequeue-column-rq_newname' )->text(),
				'rq_wiki' => $this->msg( 'globalrenamequeue-column-rq_wiki' )->text(),
				'rq_requested_ts' => $this->msg( 'globalrenamequeue-column-rq_requested_ts' )->text(),
				'row_actions' => $this->msg( 'globalrenamequeue-column-row_actions' )->text(),
			);
			...
			if ( $this->showClosedRequests() ) {
				...
				$this->mFieldNames += array(
					'rq_completed_ts' => $this->msg( 'globalrenamequeue-column-rq_completed_ts' )->text(),
					'rq_status' => $this->msg( 'globalrenamequeue-column-rq_status' )->text(),
					'rq_performer' => $this->msg( 'globalrenamequeue-column-rq_performer' )->text(),
					// TODO: link to full request detail
				);
			}
		}
